...
SELECT CONCAT( FLOOR( AVG(length) / 60) , ':' , ROUND( AVG(length) % 60) ) AS average_duration FROM sakila.film;

-- 2. 2.1 Calculate the number of days that the company has been operating. Hint: To do this, use the rental table, and the DATEDIFF() function to subtract the earliest date in the rental_date column from the latest date.
...
SELECT SUM(ISNULL(rental_duration)) FROM sakila.film; -- none are null
SELECT title,
CASE
...
SELECT 
CONCAT( first_name, ' ' , last_name, ' @ ' , LEFT( email , 3 ) ) AS address 
